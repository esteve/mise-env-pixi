...
| `activation_scripts` | Source pixi activation scripts and merge their environment changes (see [Activation Scripts](#activation-scripts)) | `false` |
...
[env]
# Source activation scripts defined in pixi.toml
_.pixi-env = { tools = true, activation_scripts = true }
```
...
By default, these scripts are not sourced — `pixi shell-hook --json` exports environment variables and
PATH entries but does not execute them. Enable `activation_scripts = true` to source the scripts and
capture any new or changed environment variables they produce:

```toml
[env]
_.pixi-env.activation_scripts = true
```
...
continues normally. The option defaults to `false`.
